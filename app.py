...
def chamar_ia_com_fallback(prompt, fallback_tipo="ecolalia"):
    ...
    if fallback_tipo == "ecolalia":
        return (
            "🔍 **Tradução Comportamental:** (Modo Contingência - Cota ou Conexão da IA Temporariamente Indisponível)\n"
            ...
            if btn_analisar:
                if not frase.strip():
                    ...
                    with st.spinner("IA Especialista em TEA analisando o contexto comportamental..."):
                        ...
                        if em_contingencia:
                            st.warning("⚠️ Cota gratuita da API esgotada temporariamente ou erro de conexão. Gerada resposta de contingência especializada.")
            ...
            if st.button("🤖 Sugerir Estratégia de Ação com IA", type="secondary", use_container_width=True):
                if not comportamento_in.strip():
                    ...
                    with st.spinner("IA calculando melhor estratégia de acolhimento e regulação..."):
                        ...
                        if em_contingencia:
                            st.warning("⚠️ Cota gratuita da API esgotada temporariamente ou erro de conexão. Carregada estratégia de contingência padrão.")
