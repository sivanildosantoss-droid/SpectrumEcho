...
# -----------------------------------------------------------------------------
# CONEXÃO COM O GEMINI (LEITURA DIRETA E BLINDADA DE SECRETS)
# -----------------------------------------------------------------------------
api_key = ""
try:
...
def chamar_ia_com_fallback(prompt, fallback_tipo="ecolalia"):
    # Se o cliente existir, tenta chamar diretamente sem travamentos desnecessários
    if client:
        ...
        for mod in modelos:
            try:
                ...
                erro_str = str(e)
                # Se for erro real de cota excedida (429), podemos tratar, senão tenta o próximo modelo
                time.sleep(0.2)
                continue

    # Fallback apenas se absolutamente todas as tentativas falharem
    if fallback_tipo == "ecolalia":
        return (
            "🔍 **Tradução Comportamental:** (Modo Contingência - Falha de Comunicação com a API)\n"
...
if st.session_state.page == "Inicio":
    ...
    if st.session_state.user_email:
        ...
        if st.button("💾 Salvar Registro", type="primary"):
            st.session_state.sensorial_db.append({"email": current_user, "gatilho": gatilho_in, "comportamento": comportamento_in, "intensidade": intensidade_in, "estrategia": "Salvo"})
            atualizar_e_salvar()
    ...
    if st.session_state.user_email:
        st.metric("Total de Ecolalias", len([e for e in st.session_state.ecolalias_db if is_admin or e.get("email") == current_user]))